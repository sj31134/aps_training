...
- **Windows와 Mac의 가상환경은 호환되지 않습니다** (Windows: `aps\Scripts\`, Mac: `aps/bin/`)
...
**중요**: 받은 파일에 가상환경 폴더(`aps/`)가 포함되어 있어도 무시하고, 각자의 환경에서 독립적으로 가상환경을 재생성해야 합니다.
...
- **가상환경 폴더는 절대 전달하지 않기** (각 환경에서 독립적으로 생성)
...
5. **가상환경 분리**: `aps/` 폴더는 전달하지 않음, `requirements.txt`로 공유
   - Windows: `aps\Scripts\activate` (Scripts 폴더)
   - Mac: `source aps/bin/activate` (bin 폴더)
   - 각 운영체제에서 독립적으로 생성하지만 동일한 패키지 설치
6. **변경 내역 문서화**: 무엇을 변경했는지 기록

### 작업 흐름

```
[GitHub 사용 작업자 (Mac/Windows)] 
    ↓ (최신 코드 전달, requirements.txt 포함, aps/ 폴더 제외)
[폐쇄된 환경 작업자 (Windows)]
    ↓ (로컬에서 가상환경 생성: python -m venv aps)
    ↓ (Windows 경로: aps\Scripts\activate)
...
[GitHub 사용 작업자 (Mac/Windows)]
    ↓ (각자 환경에서 가상환경 재생성)
    ↓ (Mac: source aps/bin/activate, Windows: aps\Scripts\activate)
    ↓ (가상환경에서 테스트)
    ↓ (GitHub에 반영)
[GitHub]
    ↓ (다른 작업자들이 pull)
[다른 작업자들 (Mac/Windows)]
    ↓ (각자 환경에서 가상환경 생성)
    ↓ (Mac: aps/bin/, Windows: aps\Scripts\)
    ↓ (각자 가상환경에서 pip install -r requirements.txt)
```

**핵심**: 가상환경은 각 운영체제에서 독립적으로 생성되지만, `requirements.txt`로 동일한 패키지를 설치합니다.
