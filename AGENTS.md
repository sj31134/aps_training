...
- **Node.js**로 개발하므로, HTML, CSS, JS 파일만 작성합니다. 프론트엔드(클라이언트 사이드)와 백엔드(서버 사이드) 모두 Node.js 기반으로 유지합니다.
...
- Node.js 환경 구축 (프론트엔드 및 백엔드 모두)
- 기본 프로젝트 구조 생성 (public/, server/, routes/ 등)
- 데이터베이스 설정 (필요시)
- 환경 변수 구성 (.env 파일)
...
### 테스트
- 프론트엔드: 브라우저 콘솔로 로그 확인
- 백엔드: API 엔드포인트 테스트 (Postman)
- 통합 테스트: 프론트엔드-백엔드 연동
- 기본 기능 수동 테스트
...
- 프론트엔드/백엔드 변경사항 구분하여 설명
- 변경사항에 대해 한글로 설명
- 디버깅 로그 포함하여 공유
- API 변경 시 프론트엔드 영향도 확인

### 놓친 부분 보충
- **에러 처리**: 프론트엔드/백엔드 모두 try-catch 추가
- **성능 최적화**: 파일 청크 로드, API 응답 캐싱
- **반응형 디자인**: 모바일 지원 고려
- **접근성**: WCAG 가이드라인 준수
